...
public sealed class UserService : IUserService
{
    ...
    public async Task<UserResponse?> UpdateAsync(UpdateUserCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            AdminUser admin => admin with
            {
                FirstName = command.FirstName,
                LastName = command.LastName,
                Email = command.Email,
                PermissionLevel = command.PermissionLevel ?? string.Empty
            },
            CustomerUser customer => customer with
            {
                FirstName = command.FirstName,
                LastName = command.LastName,
                Email = command.Email,
                LoyaltyCode = command.LoyaltyCode ?? string.Empty
            },
            EmployeeUser employee => employee with
            {
                FirstName = command.FirstName,
                LastName = command.LastName,
                Email = command.Email,
